...
int quad_contain(quad_node *r, point *p) {
...
int pair_contain(pair *p, point *a, point *b) {
...
double check_query(wspd *w, point *a, point *b) {
    ...
    assert(p != NULL);
    double e = sqrt(dist2(a, b)) / sqrt(dist2(p->u->data, p->v->data));
    return e;
}
...
double diam(point *input, int N, double eps) {
    ...
    printf("%lf [%d]\n", (double) w->tail / N / (N - 1), N);
    printf("%lf [%lf]\n", check_wspd(w, input, N), eps);
    int i;
    double ret = -1;
    for (i = 0; i < w->tail; i ++) {
...
double nearest(point *input, int N) {
    wspd *w = new_wspd(1.99, input, N);
    printf("%lf [%d]\n", (double) w->tail / N / (N - 1), N);
...
double _nearest(point *input, int N) {
    int i, j;
    double ret = -1;
    for (i = 0; i < N; i ++) {
        for (j = i + 1; j < N; j ++) {
...
void test() {
    ...
    printf("%lf\n", nearest(input, N) / _nearest(input, N));
    free(input);
}
