...
void shortest_paths_from(node *source, sp_vector *sp) {
    network *net = sp->net;
    heap_node *heap = NULL;
...
void print_distance(node *s, node *t, sp_vector *sp) {
    ...
    double lat0, double lon0, double lat1, double lon1) 
{
    ...
    sp_vector *sp = new_sp_vector(net);
    init_sp_vector(sp);
    shortest_paths_from(s0, sp);
    print_distance(s0, t0, sp);
